...
public class Empleado_DAO implements Patron_DAO<Empleado_DTO>{
	...
	private static final String SQL_UPDATE = "UPDATE empleados SET id_tipo_empleado = ?, nombre = ?, apellido = ?, sueldo = ?, dni = ?, direccion = ?, ciudad = ?, fecha_entrada = ? WHERE id_empleado = ?";
	//BUSQUEDAS
	...
	private static final String SQL_EMPLEBYTIPO = "SELECT * FROM empleados where idTipoEmpleado IN (select id_tipo from tipo_empleado where nombre_tipo= ?)";
	//private static final String SQL_FINDBYFILTRO = "SELECT * FROM empleados where Nombre like ? AND Apellido like ? AND Sueldo like ? AND DNI like ? AND Gerente = ? AND FechaEntrada like ?";
	...
	public ArrayList<Empleado_DTO> listarTodos() {
        ...
        try {
            ...
            while (rs.next()==true) {
            	Empleado_DTO _emp = new Empleado_DTO(rs.getInt(1),rs.getString(2),rs.getString(3), rs.getDouble(4), rs.getString(5), rs.getString(6), rs.getString(7), rs.getDate(8)); 
